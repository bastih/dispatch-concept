...
//  ------------
//
//  This design evolved out of another design where the behavior of software
// system was specified
//  using various type sequences. A generic implementation needed to be
// overloaded for specific
//  combinations of types and the search for a way to do that indicated the need
// for such an
//  algorithm.
//
//  Another use for the algorithm was in testing a complex class where specific
// functionality of
//  that class could be specified in external functors that are then passed as
// type parameters to
//  the class template.
//
//  Using cartesian_product is an easy way to test various configurations in a
// common manner.
//
...
//  };
//
//  //  Test function that creates a complex class from a type sequence and then
// tests it
//  template <typename Sequence>
//  void
//  test_complex_class()
...
//  If done == true, then the end of the sequence has been reached and nothing
// needs to be done
...
struct cartesian_product_inner_impl<false> {
  ...
  static void execute(CurrentType *, LastType *, CurrentSequence *, LastSequence *,
    ...
    cartesian_product_inner_impl<boost::is_same<NextType, LastType>::value>::execute(
        (NextType *)0, (LastType *)0, (CurrentSequence *)0, (LastSequence *)0,
...
//  The specialization takes care of the case where the recursion is NOT yet
// done
//
//  This is called for every outer sequence
template <>
struct cartesian_product_outer_impl<false> {
  template <typename CurrentSequence,   //  Iterator to current outer sequence
  ...
  static void execute(CurrentSequence *, LastSequence *, ArgumentSequence *, F f) {
    ...
    cartesian_product_inner_impl<done_type::value>::execute((FirstType *)0, (LastType *)0, (NextSequence *)0, (LastSequence *)0, (ArgumentSequence *)0, f);
  }
};
...
template <typename SequenceOfSequences, typename F>
inline void cartesian_product(F f, SequenceOfSequences * = 0) {
