...
class strong_typedef : boost::totally_ordered1<strong_typedef<T>, boost::totally_ordered2<strong_typedef<T>, T> > {
 public:
  ...
  strong_typedef(const strong_typedef& t_) : value(t_.value) {}

  strong_typedef(const strong_typedef&& t_) : value(std::move(t_.value)) {}

  constexpr explicit strong_typedef(const T val) : value(val) {};
  ...
  bool operator==(const strong_typedef& rhs) const { return value == rhs.value; }
