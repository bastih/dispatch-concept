...
class DefaultValueCompressedStorage : public AStorage {
 public:
  ...
  void createPositionList(value_id_t vid, std::size_t offset, std::vector<std::size_t>& positions) {
    if (likely(vid == _default_value)) createPositionListFromDefault(offset, positions);
    else createPositionListForException(vid, offset, positions);
  }
  ...
  void createPositionListFromDefault(std::size_t offset, std::vector<std::size_t>& positions) const {
    ...
    for (const auto& range: ranges) {
        std::iota(positions.begin()+offset+range._start, positions.begin()+offset+range._stop, range._start + offset);
    }
  }
